...
class UUIDValueChangerTest {
	...
	final UUID uuid = UUID.randomUUID();
	return Stream.of(new TestCase(null, null, false), new TestCase(uuid, uuid, false), new TestCase(uuid, null, true), new TestCase(null, uuid, true), new TestCase(uuid, UUID.randomUUID(), true)).map(value -> dynamicTest(getDefaultDisplayName(value.value1 + " " + value.value2), Should_Return_True_Or_False_Whether_Values_Are_Different_Or_Not(value)));
    }
