...
public class FSTORE extends Instruction {
    ...
    public FSTORE(int n, LocalVariableTableGen localVariableTable, ConstantPool cp) {
        super(InstructionOpCodes.FSTORE_0.add(n), 1, cp);
        this.index = n;
        this.localVariableTable = localVariableTable;
        this.localVariable = localVariableTable.getLocalVariableTable()[index];
    }

    public FSTORE(LocalVariableTableGen localVariableTable, ConstantPool cp) {
        super(InstructionOpCodes.FSTORE, 1, cp);
        this.localVariableTable = localVariableTable;
    }
    ...
    public void initFromFile(ByteSequence bytes, boolean wide) throws IOException {
        if (wide) {
            ...
            super.setLength(4);
        } else if (opcode == InstructionOpCodes.FSTORE) {
