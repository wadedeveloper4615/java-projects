...
class StringValueChangerTest {
	...
	return Stream.of(new TestCase(null, null, false), new TestCase("", "", false), new TestCase("same", "same", false), new TestCase("", null, true), new TestCase(null, "", true), new TestCase("", "test", true)).map(value -> dynamicTest(getDefaultDisplayName(value.value1 + " " + value.value2), Should_Return_True_Or_False_Whether_Values_Are_Different_Or_Not(value)));
    }
