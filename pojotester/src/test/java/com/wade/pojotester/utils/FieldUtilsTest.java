...
import com.wade.pojotester.testfiles.fields.ClassWithAllAvailableFieldModifiers;
import com.wade.pojotester.testfiles.fields.Permutation1;
import com.wade.pojotester.testfiles.fields.Permutation2;
...
class FieldUtilsTest {
	...
	// given
	final List<String> expectedFields = TestHelper.getAllFieldsExceptDummyJacocoField(ClassWithAllAvailableFieldModifiers.class).stream().map(Field::getName).collect(Collectors.toList());
	// when
	...
	final TestCase testCase1 = new TestCase(Permutation1.class, newArrayList(newArrayList(perm1A), newArrayList(perm1B), newArrayList(perm1A, perm1B)));
	final TestCase testCase2 = new TestCase(Permutation2.class, newArrayList(newArrayList(perm2A), newArrayList(perm2B), newArrayList(perm2C), newArrayList(perm2A, perm2B), newArrayList(perm2A, perm2C), newArrayList(perm2B, perm2C), newArrayList(perm2A, perm2B, perm2C)));
	return Stream.of(testCase1, testCase2).map(value -> dynamicTest(getDefaultDisplayName(value), Should_Return_All_Permutations(value)));
    }
	...
	return () -> {
	    ...
	    // then
	    assertThat(permutations).hasSameSizeAs(testCase.fields).containsAll(testCase.fields);
	};
    }
	...
	// then
	assertThat(result).hasSize(1).containsExactly(expectedField);
    }
